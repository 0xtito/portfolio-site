...
async function addNftsToDB() {
  ...
  if ((await nftsCollection.estimatedDocumentCount()) == 0) {
    ...
    fs.readFile(filePathAtlanta, async (err, data) => {
      // console.log(data, err);
      const binaryObj = new Binary(data)
      let complete = await gifCollection.insertOne({ title: "atlanta", gif: binaryObj})
      console.log(complete.insertedId);
    })

    fs.readFile(filePathKafka, async (err, data) => {
      // console.log(data, err);
      const binaryObj = new Binary(data)
      let complete = await gifCollection.insertOne({ title: "kafka", gif: binaryObj})
      console.log(complete.insertedId);
    })

    // const res = await gifCollection.findOne({})
    // console.log(Buffer.from(res.gif).toString('base64'));
...
function filterNfts(_rawNftsArr, isOrdered) {
  ...
  for (let nft of rawNftsArr) {
    ...
    } else if (nft.isAnimated) {
      let animationUrl = nft.metadata.animation_url;
    }
